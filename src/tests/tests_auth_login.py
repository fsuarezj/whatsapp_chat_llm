...
def test_login_missing_data(client):
    # No data None
    resp = client.post('/login', json=None)
...
def test_login_invalid_credentials(client):
    ...
    # Wrong username
    resp = client.post('/login', json={'username': 'user2', 'password': 'Password123'})
    assert resp.status_code == 401
    # Wrong password
...
def test_login_resets_rate_limit_on_success(client):
    ...
    # 2 failed attempts
    for _ in range(2):
        client.post('/login', json={'username': 'user1', 'password': 'WrongPass'})
    # Successful login
    resp = client.post('/login', json={'username': 'user1', 'password': 'Password123'})
    assert resp.status_code == 200
